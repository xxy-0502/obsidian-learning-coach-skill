...
合并后的 `full.md` 会按 part 标注页码范围。临时 `parts/` 目录默认会在合并成功后清理；如果你需要调试每个分卷的 PDF 和 Markdown，可以加 `--keep-parts`。若你不想自动切分，可以加 `--no-auto-split`。
...
- 临时 `parts/` 目录会在成功合并后自动清理；使用 `--keep-parts` 才会保留。
...
对于教材、书籍、长报告、上百页 PDF 这类资料，推荐直接使用一键准备流程。它会自动完成转换、结构分析、章节索引，并在成功合并后清理临时分卷文件：
...
```text
LearningVault/inbox/converted/[资料名]/chapter_index.md
LearningVault/inbox/converted/[资料名]/chapters/
```

不是所有资料都需要拆分。转换完成后，长资料或结构复杂的资料也可以手动运行结构分析：
...
- `scripts/prepare_source.py`：一键准备长资料，串联转换、结构分析、章节拆分和临时文件清理。
